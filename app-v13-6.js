...
function backupPayload(){
  return {app:"Minha Loja",version:"V13.2",createdAt:new Date().toISOString(),data:{products:db.products||[],sales:db.sales||[],expenses:db.expenses||[],users:db.users||[]}};
}
...
(function(){
  ...
  const uid=s=>String(s?.saleGroupId||s?.groupId||s?.id||'');
  const cancelled=s=>s?.status==='cancelled'||s?.cancelled===true;
  const currentUser=()=>{
    try{return JSON.parse(sessionStorage.getItem('minha_loja_login_session')||localStorage.getItem('minha_loja_login_session')||'null');}catch(e){return null;}
  };
  const isAdmin=()=>currentUser()?.role==='admin';

  function productName(s){return String(s?.productName||s?.name||s?.product?.name||s?.product?.nome||'Produto');}
  function productId(s){return String(s?.productId??s?.product?.id??'');}
  function quantity(s){return Math.max(0,Number(s?.quantity??s?.qty??s?.quantidade??1)||0);}
  function stockKey(p){
    for(const k of ['stock','estoque','quantity','quantidade']) if(Object.prototype.hasOwnProperty.call(p,k)) return k;
    return 'stock';
  }
  ...
  window.cancelSaleV136=function(groupId){
    ...
    const d=getDB();
    if(!Array.isArray(d.sales)||!Array.isArray(d.products)){alert('Dados de vendas/estoque não encontrados.');return;}
    const gid=String(groupId);
    const rows=d.sales.filter(s=>uid(s)===gid);
    if(!rows.length){alert('Venda não encontrada.');return;}
    if(rows.every(cancelled)){alert('Essa venda já está cancelada.');return;}
    ...
    rows.forEach(s=>{
      const pid=productId(s), name=productName(s), q=quantity(s);
      const p=d.products.find(x=>String(x?.id??'')===pid) ||
              d.products.find(x=>String(x?.name??x?.nome??'')===name);
      if(p && q){
        const k=stockKey(p);
        p[k]=Math.max(0,(Number(p[k])||0)+q);
      }
      s.status='cancelled';
      s.cancelled=true;
  ...
  // Adds an action to any history element carrying a sale group/id.
  function decorate(){
    if(!isAdmin()) return;
    document.querySelectorAll('[data-sale-group-id],[data-sale-id],[data-group-id]').forEach(el=>{
      if(el.dataset.v136Decorated)return;
      const gid=el.dataset.saleGroupId||el.dataset.saleId||el.dataset.groupId;
      if(!gid)return;
      el.dataset.v136Decorated='1';
      if(el.querySelector('.v136-cancel'))return;
      const b=document.createElement('button');
      b.type='button'; b.className='v136-cancel';
      b.textContent='Cancelar venda';
      b.style.cssText='margin:8px 0 0;padding:8px 12px;border:0;border-radius:8px;cursor:pointer;';
      b.onclick=()=>window.cancelSaleV136(gid);
      el.appendChild(b);
    });
  }
  function boot(){
