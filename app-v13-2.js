...
function openSaleForm(){
  ...
  const rows=available.map(p=>{
    ...
    return `<div class="sale-stock-item" data-product-id="${p.id}">
      <label class="sale-check-wrap">
        <input type="checkbox" class="sale-check" data-id="${p.id}">
      ...
      <div class="sale-qty-control">
        <button type="button" class="mini sale-minus" data-id="${p.id}" disabled>−</button>
        <span class="sale-qty-display" data-id="${p.id}">0</span>
        <button type="button" class="mini sale-plus" data-id="${p.id}" disabled>+</button>
      </div>
  ...
  openModal("Registrar venda",`<div class="form-grid">
    ...
    <div style="grid-column:1/-1"><div class="panel-title" style="margin-bottom:8px">Produtos disponíveis em estoque</div><div class="sale-stock-list">${rows}</div><div id="saleGrandTotal" class="auto-status" style="margin-top:12px">Total da venda: <b>${money(0)}</b></div></div>
  ...
  available.forEach(p=>{
    const id=String(p.id), row=document.querySelector(`.sale-stock-item[data-product-id="${CSS.escape(id)}"]`), stock=stockCurrent(p);
    row.querySelector('.sale-check').addEventListener('change',e=>{selected[id]=e.target.checked?1:0;refresh();});
    row.querySelector('.sale-minus').addEventListener('click',()=>{selected[id]=Math.max(0,Number(selected[id]||0)-1);refresh();});
    row.querySelector('.sale-plus').addEventListener('click',()=>{selected[id]=Math.min(stockCurrent(p),Number(selected[id]||0)+1);refresh();});
  });
  refresh();
}
