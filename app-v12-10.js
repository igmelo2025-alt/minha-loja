...
function saleRowHtml(index=0){
  ...
  return `<div class="sale-line" data-sale-line="${index}">
    <div class="field"><label>Produto</label><select name="productId" class="select sale-product" required>${opts}</select></div>
    <div class="field"><label>Quantidade</label><input name="qty" class="sale-qty" type="number" min="1" step="1" value="1" required></div>
    <div class="field"><label>Preço/un.</label><input name="price" class="sale-price" type="number" min="0" step="0.01" required></div>
    <button type="button" class="mini sale-remove" title="Remover item" style="display:${index?'inline-block':'none'}">Remover</button>
  </div>`;
}
...
function renderUsers(){
  const me=document.getElementById("currentUserInfo"); if(me) me.innerHTML=currentUser?`Conectado como <b>${esc(currentUser.name||currentUser.username)}</b> · ${currentUser.role==='admin'?'Administrador':'Usuário'}`:'';
  const el=document.getElementById("usersList"); if(!el)return;
  if(!isAdmin()){el.innerHTML='<div class="empty">Somente o administrador pode gerenciar usuários.</div>';return;}
  el.innerHTML=db.users.map(u=>`<div class="item"><div><div class="item-title">${esc(u.name||u.username)}</div><div class="item-sub">@${esc(u.username)} · ${u.role==='admin'?'Administrador':'Usuário'}</div></div><div class="item-actions"><button class="mini" onclick="editUser('${u.id}')">Editar</button>${u.id!=='admin'?`<button class="mini" onclick="deleteUser('${u.id}')">Excluir</button>`:''}</div></div>`).join('');
}
function openUserForm(id){
  if(!isAdmin()){alert('Acesso permitido somente ao administrador.');return;}
  const u=db.users.find(x=>x.id===id)||{id:'',username:'',name:'',role:'user'};
  openModal(id?'Editar usuário':'Novo usuário',`<div class="form-grid"><div class="field"><label>Nome</label><input name="name" value="${esc(u.name||'')}" required></div><div class="field"><label>Usuário</label><input name="username" value="${esc(u.username||'')}" required autocomplete="off"></div><div class="field"><label>Perfil</label><select name="role" class="select" ${u.id==='admin'?'disabled':''}><option value="user" ${u.role!=='admin'?'selected':''}>Usuário</option><option value="admin" ${u.role==='admin'?'selected':''}>Administrador</option></select></div><div class="field"><label>${id?'Nova senha (deixe em branco para manter)':'Senha'}</label><input name="password" type="password" minlength="4" ${id?'':'required'} autocomplete="new-password"></div><div class="form-actions"><button type="button" class="secondary" onclick="closeModal()">Cancelar</button><button class="primary">Salvar usuário</button></div></div>`,async fd=>{
    const username=fd.get('username').trim().toLowerCase(),name=fd.get('name').trim(),role=u.id==='admin'?'admin':fd.get('role'),password=fd.get('password');
    ...
    const obj={id:u.id||uid(),username,name,role,passwordHash:u.passwordHash||''};
    ...
    if(currentUser?.id===id){currentUser={...currentUser,...obj};try{sessionStorage.setItem('minha_loja_current_user',JSON.stringify(currentUser));}catch(e){}}
    return save();
  });
}
function editUser(id){openUserForm(id)}
function deleteUser(id){if(!isAdmin()||id==='admin')return;const u=db.users.find(x=>x.id===id);if(u&&confirm(`Excluir o usuário "${u.username}"?`)){db.users=db.users.filter(x=>x.id!==id);save();}}
...
function backupPayload(){
  return {app:"Minha Loja",version:"V12.10",createdAt:new Date().toISOString(),data:{products:db.products||[],sales:db.sales||[],expenses:db.expenses||[],users:db.users||[]}};
}
...
// V12.10 — acesso e usuários
document.getElementById("addUserBtn")?.addEventListener("click",()=>openUserForm());
document.getElementById("logoutBtn")?.addEventListener("click",logout);
window.onMinhaLojaLogin=function(user){currentUser=user;renderUsers();};
