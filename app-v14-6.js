...
function downloadSimplePdf(lines,filename){
  ...
  const wrapped=[]; lines.forEach(line=>{let t=clean(line); if(!t){wrapped.push("");return;} while(t.length>82){wrapped.push(t.slice(0,82));t=t.slice(82);} wrapped.push(t);});
  const perPage=48,pages=[]; for(let i=0;i<wrapped.length;i+=perPage)pages.push(wrapped.slice(i,i+perPage)); if(!pages.length)pages.push([""]);
...
function openPrintWindow(content,title){
  ...
  style.textContent=`@media print{body>*:not(#__minhaLojaPrintArea){display:none!important}#__minhaLojaPrintArea{display:block!important;position:static!important;width:100%!important;background:#fff!important;color:#111!important;padding:20px!important} @page{margin:10mm} } @media screen{#__minhaLojaPrintArea{position:fixed;z-index:999999;inset:12px;background:#fff;overflow:auto;padding:24px;box-shadow:0 4px 30px rgba(0,0,0,.25);border-radius:12px}#__minhaLojaPrintArea:before{content:'Pré-visualização de impressão';display:block;font-weight:800;margin-bottom:15px}}`;
