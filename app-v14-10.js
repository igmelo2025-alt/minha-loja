...
function printSaleReceipt(groupId){
  // No celular, abrir diretamente um único PDF compacto evita duas prévias (A4 + compacto).
  // O PDF compacto pode ser salvo ou impresso pelo visualizador do aparelho.
  ...
  lines.push("------------------------------","TOTAL: "+money(total),"Pagamento: "+(rows[0].payment||"Nao informado"),"","Obrigado pela preferencia!","","Comprovante de pedido - nao substitui nota fiscal eletronica.");
  openPdfFromLines(lines,"pedido-"+String(groupId).slice(0,8)+".pdf",{width:226.77,height:430,fontSize:12,leading:17,left:14,top:400,maxChars:30});
}
function printCompactSaleReceipt(groupId){
  // Mantido para compatibilidade com instalações antigas.
  printSaleReceipt(groupId);
}
...
function downloadCompactSalePdf(groupId){
  ...
  rows.forEach(s=>lines.push((s.qty||0)+"x "+(s.productName||"Produto"),"   "+money(Number(s.qty||0)*Number(s.price||0))));
  lines.push("------------------------------","TOTAL: "+money(total),"Pagamento: "+(rows[0].payment||"Nao informado"),"","Obrigado pela preferencia!","","Comprovante de pedido - nao substitui nota fiscal eletronica.");
  downloadSimplePdf(lines,"pedido-compacto-"+String(groupId).slice(0,8)+".pdf",{width:226.77,height:430,fontSize:12,leading:17,left:14,top:400,maxChars:30});
}
...
function openPdfFromLines(lines,filename,opts={}){
  // Gera o mesmo PDF compacto, mas abre apenas uma visualização. O usuário pode
  // escolher "Salvar como PDF" ou "Imprimir" no próprio visualizador do Android.
  const clean=v=>String(v??"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[R$€£]/g,"R").replace(/[^\x20-\x7E]/g,"");
  const escPdf=v=>clean(v).replace(/\\/g,"\\\\").replace(/\(/g,"\\(").replace(/\)/g,"\\)");
  const maxChars=opts.maxChars||34;
  const wrapped=[]; lines.forEach(line=>{let t=clean(line);if(!t){wrapped.push("");return;}while(t.length>maxChars){wrapped.push(t.slice(0,maxChars));t=t.slice(maxChars);}wrapped.push(t);});
  const perPage=opts.perPage||Math.max(1,Math.floor(((opts.height||566.93)-40)/(opts.leading||13))),pages=[];for(let i=0;i<wrapped.length;i+=perPage)pages.push(wrapped.slice(i,i+perPage));if(!pages.length)pages.push([""]);
  const objs=[];const add=x=>{objs.push(x);return objs.length;};const font=add("<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>");const pageIds=[];
  pages.forEach(pg=>{let stream=`BT\n/F1 ${opts.fontSize||9} Tf\n${opts.left||18} ${opts.top||540} Td\n`;pg.forEach((line,i)=>{if(i)stream+=`0 -${opts.leading||13} Td\n`;stream+="("+escPdf(line)+") Tj\n";});stream+="ET";const sid=add("<< /Length "+stream.length+" >>\nstream\n"+stream+"\nendstream");const pid=add("<< /Type /Page /Parent PAGES /MediaBox [0 0 "+(opts.width||226.77)+" "+(opts.height||566.93)+"] /Resources << /Font << /F1 "+font+" 0 R >> >> /Contents "+sid+" 0 R >>");pageIds.push(pid);});
  const pagesObj=add("<< /Type /Pages /Kids ["+pageIds.map(id=>id+" 0 R").join(" ")+"] /Count "+pageIds.length+" >>");const catalog=add("<< /Type /Catalog /Pages "+pagesObj+" 0 R >>");pageIds.forEach(id=>objs[id-1]=objs[id-1].replace("PAGES",pagesObj+" 0 R"));
  let pdf="%PDF-1.4\n",offsets=[0];objs.forEach((o,i)=>{offsets[i+1]=pdf.length;pdf+=(i+1)+" 0 obj\n"+o+"\nendobj\n";});const xref=pdf.length;pdf+="xref\n0 "+(objs.length+1)+"\n0000000000 65535 f \n";for(let i=1;i<=objs.length;i++)pdf+=String(offsets[i]).padStart(10,"0")+" 00000 n \n";pdf+="trailer\n<< /Size "+(objs.length+1)+" /Root "+catalog+" 0 R >>\nstartxref\n"+xref+"\n%%EOF";
  const blob=new Blob([pdf],{type:"application/pdf"});const url=URL.createObjectURL(blob);
  const w=window.open(url,"_blank");
  if(!w){alert("O navegador bloqueou a abertura do PDF. Toque em Permitir pop-ups e tente novamente.");URL.revokeObjectURL(url);return;}
