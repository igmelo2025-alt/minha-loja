  ...
  el.innerHTML=groupsArr.length?groupsArr.map(rows=>{
    ...
    return `<div class="item ${cancelledGroup?"v136-cancelled-item":""}">
      <div>
        ...
        <div class="item-sub">${dateBR(rows[0].date)} · ${payment}</div>
...
async function pushCloudV138(showAlert=true){
  ...
  try{
    ...
    const r=await fetch(url+'/rest/v1/app_backups',{method:'POST',headers:{'Content-Type':'application/json','apikey':key,'Authorization':'Bearer '+key,'Prefer':'return=minimal'},body:JSON.stringify({backup:payload,created_at:payload.updatedAt})});
...
async function fetchCloudV138(){
  ...
  const r=await fetch(url+'/rest/v1/app_backups?select=backup,created_at&order=created_at.desc&limit=50',{headers:{apikey:key,Authorization:'Bearer '+key}});
  if(!r.ok)throw new Error(await r.text());
